...
`--batch-size N`, `--timeout S`, and `--jobs N`. The roster-resolved advisory route is
selected from the current `opencode-go` standard-tier roster at runtime; Quizzler has
no model fallback.
...
roster-resolved advisory evidence and one complete high-capability-verifier census from
...
report blocks the campaign; operational or incomplete evidence from the roster-resolved
advisory route remains advisory. An operational failure may retry only on the unchanged snapshot; any
...
|---|---|---|
| `external-layer-c-strict` | Configured high-capability verifier inside `hybrid_verify.py` | The project's designated external critic reviewed the pack; the roster-resolved advisory review is advisory evidence only |
...
- **Run it via the campaign.** `scripts/hybrid_verify.py --no-certify` runs
  the roster-resolved advisory route then the configured high-capability verifier as discovery evidence;
